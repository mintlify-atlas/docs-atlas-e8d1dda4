---
title: "Idempotent Booking: How Slot Reservation Works"
sidebarTitle: "Booking Flow"
description: "How the booking endpoint survives network retries and 10-way race conditions using a Redis replay cache, PostgreSQL advisory locks, and SELECT FOR UPDATE SKIP LOCKED."
---
...
Booking requests are limited to **30 per minute per user** via a Redis fixed-window counter keyed as `rl:booking:{user_id}`. The counter is incremented with `INCR` and a 60-second `EXPIRE` is set on the first hit. The limiter **fails open** on Redis outage — Nginx edge rate limits still apply.
